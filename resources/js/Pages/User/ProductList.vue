...
import {
    ...
    TransitionChild,
    TransitionRoot,
} from '@headlessui/vue' //headlessui template
...
const priceFilter = () => {
    filterPrices.transform((data) => ({
        ...data, // titik 3 artinya mengurai
        prices: {
            from: filterPrices.prices[0],
            to: filterPrices.prices[1]
        } //ini akan membuat bject baru daru array data yg diurai dan akan membuat obejct price : {from:..,to:..}
        // dan akan dikirim ke product yg mana di produucts akan di tangkap oleh filter yg sudah kita buat model
    })).get('products', {
        preserveState: true, //Ini menandakan bahwa state saat ini akan dipertahankan setelah operasi ini selesai.
        replace: true //artinya data akan digantikan dengan yg baru
    })
}
...
watch(selectedBrands, () => {
    updateFilteredProducts() //watch berfungsi mengawasi setiap ada perubahan pada seleectedBrands dan kemudian akan melakukan function setiap ada perubahan
})
...
function updateFilteredProducts() {
    ...
    }, { //ini akan dikirim ke products yg mana sudah kita buatkan filter dimodelnya
        preserveState: true,
        replace: true
    })
}
