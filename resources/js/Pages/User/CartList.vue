...
const carts = computed(() => usePage().props.cart.data.items) // ini kita akan ambil data items apa aja sih yg masuk di dalam cart
const products = computed(() => usePage().props.cart.data.products) // ini detail product yg masuk

const total = computed(() => usePage().props.cart.data.total) //total seluruh harga di cart
const itemId = (id) => carts.value.findIndex((item) => item.product_id === id);

const form = reactive({ //ini hampir sam dengan ref
    address1: null,
    state: null,
    city: null,
    zipcode: null,
...
const update = (product, quantity) =>
    router.patch(route('cart.update', product), {
        quantity, //disini kita akan kirim updated yg mana quantity akn berubah
    });
//remove form cart 
const remove = (product) => router.delete(route('cart.delete', product)); //delete product


//confirm order 

function submit() { //ini akan kita submit ke route checkout.store yg kita buat untuk melakukan pembayaran
    router.visit(route('checkout.store'), {
        method: 'post', //pakai post soalnya akan megirim inputan order
        data: {
    ...
    <UserLayout>
        <section class="text-gray-600 body-font relative">
            <div class="container px-5 py-24 mx-auto flex sm:flex-nowrap flex-wrap">
                <div class="lg:w-2/3 md:w-1/2  rounded-lg  sm:mr-10 p-10 overflow-x-scroll">
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            <tr v-for="product in products" :key="product.id"
                                ...
                                <td class="w-32 p-4">
                                    <img v-if="product.product_images[0] != null"
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        v-if="carts[itemId(product.id)].quantity > 1"
                  ...
                  <div v-else>
                    ...
                    <form @submit.prevent="submit">
                        <div class="relative mb-4">
                            ...
                            <input required type="text" id="name" name="address1" v-model="form.address1"
                                class="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                        </div>
                        <div class="relative mb-4">
                            <label for="email" class="leading-7 text-sm text-gray-600">City</label>
                            <input required type="text" id="email" name="city" v-model="form.city"
                                class="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                        </div>
                        <div class="relative mb-4">
